...
def prepare_features(df):
    ...
    # Guard: ensure non-negative result even if sleep_quality_score exceeds scale maximum
    X['sleep_deprivation_index'] = np.maximum(
        0,
        np.maximum(0, 8.0 - X['sleep_hours_numeric']) * (4.0 - X['sleep_quality_score'])
    )
